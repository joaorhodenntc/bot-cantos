...
async function obterPartidas() {
    const url = "https://apiv3.apifootball.com/?action=get_events&match_live=1&APIkey=9d7d143e9b45512e4473d54be44631098c85627a6abe90fbe6ece48f7824e5f8";
    const response = await axios.get(url);
    return response.data;
}

async function obterOdds(idPartida){ 
    const url = `https://apiv3.apifootball.com/?action=get_odds&APIkey=9d7d143e9b45512e4473d54be44631098c85627a6abe90fbe6ece48f7824e5f8&match_id=${idPartida}`
    const response = await axios.get(url);
    return response.data;
}
...
function casaFavoritoPressao(apHome, scoreHome, scoreAway, idPartida, partidasNotificadas, minutes, chutesHome){
    if((apHome/minutes>= 1.20) && (scoreHome==scoreAway || (scoreAway - scoreHome)==1) && chutesHome>=15 &&!partidasNotificadas.has(idPartida)){
        return true
    }
}

function foraFavoritoPressao(apAway, scoreHome, scoreAway, idPartida, partidasNotificadas, minutes, chutesAway){
    if((apAway/minutes>= 1.20) && (scoreHome==scoreAway || (scoreHome - scoreAway)==1)  && chutesAway>=15 && !partidasNotificadas.has(idPartida)){
        return true
    }
}
...
async function analisarPartidas(){
    ...
    for(let i=0; i<qtdPartidas; i++){
        ...
        if(minutes!='Finished' && ((minutes>=79 && minutes<=84))){
...
app.get('/cantos', (req, res) => {
    ...
    res.send("<b>BOT CANTO</b><br>"+ " 🚨 "+ qtdPartidas + " Jogos ao vivo<br>"+" 🤖 Analisando " + partidasEmAnalise.size + " Partidas<br>" + " 💾 Partidas Notificadas: ["+ [...partidasNotificadas].join(", ")+"]<br>" + " ⏰ Hora atual: " + horaAtual);
});
